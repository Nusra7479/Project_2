...
def parse_json(json_obj):
    q = queue.Queue()
    q_node = queue.Queue()
    plan = json_obj['Plan'] # plan = json_obj[0]['Plan']
    q.put(plan)
    q_node.put(None)
...
def simplify_graph(node):
    ...
    for child in node.children:
        ...
        if child.actual_time is not None:
            if new_node.actual_time is not None:
                new_node.actual_time -= child.actual_time
...
def generate_why(node_a, node_b, num):
    ...
    if node_a.node_type == "Index Scan" and node_b.node_type == "Seq Scan":
        ...
        if int(node_a.actual_rows) < int(node_b.actual_rows):
            text += "and the actual row number increases from " + str(node_a.actual_rows) + " to " + str(
                node_b.actual_rows) + ", "
        ...
        if node_a.node_type == "Merge Join" and node_b.node_type == "Hash Join":
            ...
            if int(node_a.actual_rows) < int(node_b.actual_rows):
                text += "This is because the actual row number increases from " + str(
                    node_a.actual_rows) + " to " + str(node_b.actual_rows) + ". "
            if int(node_a.actual_rows) > int(node_b.actual_rows):
                text += "The actual row number decreases from " + str(node_a.actual_rows) + " to " + str(
                    node_b.actual_rows) + ". "
            text += "The both side of the Join operator of P2 can be sorted on the join condition efficiently . "
